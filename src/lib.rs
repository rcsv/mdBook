...
//! > behavior are backwards compatible, the tool's internals are still
//! > evolving and being iterated on. If you with to prevent accidental
...
//! be placed in. For a much more in-depth explanation, consule the [relevant
...
// これはどういう意味でしょうねぇ。
// deny() は何か、エラーを出すようです。この場合、missing_docsがなんなのかわからんですけど
#![deny(missing_docs)]

// macro_useという属性は、次に来るcrateのマクロが全てロードされるようです。使わないと
// ロードされないということでしょうか。この場合、error_chain のマクロが全てロードされ
// るということだと解釈します。この辺りはわざわざ Cargo.toml で事前に使うよと宣言してい
// る外部クレートですね。
#[macro_use]
...
// このcfgは、続くpretty_assertionsに作用しているものと思われます。多分
// pretty_assertions内部でtestという変数が宣言されている必要があるのかも
#[cfg(test)]
#[macro_use]
extern crate pretty_assertions;

// モジュールの宣言。名前を宣言するだけで、Rustの規則から、本体のソースコードは
// 大体 book.rs か、book/mod.rs のどちらかだろうと予想してビルドが行われる
// ようです。ここでは book, config, preprocess, renderer, theme, utils 
// というディレクトリとその下に mod.rs を作ることになります。
pub mod book;
pub mod config;
pub mod preprocess;
pub mod renderer;
