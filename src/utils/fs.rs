use errors::* ;
use std::fs::{ self, File}  ; // will use filesystem standard module
...
/// Naively replaces any path separatoro with a forward-slash '/'
pub fn normalize_path(path: &str) -> String {
    use std::path::is_separator ;
    path.chars()
        .map(|ch| if is_separator(ch) { '/' } else { ch })
        .collect::<String>()
    // この中だけで、is_separator関数を使う宣言
    // |ch|がよくわからない。
    // 関数のparameterを書くところにロジックが書いてあってよくわからん
    // 最後にセミコロンがないのも、何故なんだろう？
}
